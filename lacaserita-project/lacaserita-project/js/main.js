...
function changeCartQty(id, delta) {
  ...
  item.qty = Math.round((item.qty + delta) * 10) / 10;
...
function updateCartUI() {
  ...
  // NUEVO: actualizar total visible del carrito
  ...
  qs('#cart-wa-btn').addEventListener('click', (e) => {
  e.preventDefault();
  ...
  window.open(
    buildWaLink(CONTACT.phone, message),
    '_blank'
  );
});
